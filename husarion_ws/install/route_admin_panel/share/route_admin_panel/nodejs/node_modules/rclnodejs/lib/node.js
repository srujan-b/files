...
const ActionClient = require('./action_client.js');
...
class Node {
  ...
  execute() {
    ...
    this._action_clients.forEach((action_client) => {
      let Status = action_client.typeClass.Status;
      let status = new Status();
      let success = rclnodejs.rclActionTakeStatus(action_client.handle, status.toRawROS());
      if(success){
        action_client.processStatusCallback(status.refObject);      
      }
      Status.destoryRawROS(status);
    });

    this._action_clients.forEach((action_client) => {
      let GoalRes = action_client.typeClass.GoalRes;
      let goalRes = new GoalRes();
      let success = rclnodejs.rclTakeGoalResponse(action_client.handle, action_client._sequenceNumber, goalRes.toRawROS());
      if (success) {
        action_client.processGoalResponse(goalRes.refObject);
      }
      GoalRes.destoryRawROS(goalRes);
    });

    this._action_clients.forEach((action_client) => {
      let ResultRes = action_client.typeClass.ResultRes;
      let resultRes = new ResultRes();
      let success = rclnodejs.rclTakeResultResponse(action_client.handle, action_client._resultSequenceNumber, resultRes.toRawROS());
      if (success) {
        action_client.processResultResponse(resultRes.refObject);
      }
      ResultRes.destoryRawROS(resultRes);
    });
  ...
  /**
    ...
    * @param {function|string|object} typeClass - The ROS action class,
         OR a string representing the message class, e.g. 'std_msgs/msg/String',
         OR an object representing the message class, e.g. {package: 'std_msgs', type: 'msg', name: 'String'}
    * @param {string} action - The action name to query.
    * @param {object} options - The options argument used to parameterize the client.
    * @param {function} status_callback - The method to be called when status is received.
    * @param {function} feedback_callback - The method to be called when feedback is received.
    * @return {ActionClient} - An instance of ActionClient.
    */

  createActionClient(typeClass, action, options, status_callback, feedback_callback, goal_response_callback, result_response_callback) {
    if (typeof (typeClass) === 'string' || typeof (typeClass) === 'object') {
      typeClass = loader.loadActionInterface(typeClass);
    }
    ...
    let actionClient = ActionClient.createActionClient(this.handle, typeClass, action, options, status_callback, feedback_callback, goal_response_callback, result_response_callback);
    this._action_clients.push(actionClient);
    this.syncHandles();
    return actionClient;
  }
