...
let interfaceLoader = {
  ...
  loadInterface(packageName, type, messageName) {
    ...
    throw new Error(`The message required does not exist: ${packageName}, ${type}, ${messageName}`);
  },

  loadActionInterface(name) {
    let packageName, type, actionName;
    if (name.indexOf('/') !== -1) {
      [packageName, type, actionName] = name.split('/');
      let filePath = path.join(
        generator.generatedRoot, packageName, packageName + '__' + type + '__' + actionName + '.js');
      // eslint-disable-next-line
      if (fs.existsSync(filePath)) {
        return require(filePath);
      }
    }
    throw new Error(`The message required does not exist: ${packageName}, ${type}, ${actionName}`);
  }
};
