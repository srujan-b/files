...
const LinkStateRefStruct = StructType({
...
const LinkStateRefArray = ArrayType(LinkStateRefStruct);
const LinkStateRefStructArray = StructType({
  data: LinkStateRefArray,
...
class LinkStateWrapper {
  constructor(other) {
    ...
    if (typeof other === 'object' && other._refObject) {
      this._refObject = new LinkStateRefStruct(other._refObject.toObject());
  ...
  _initMembers() {
    this._refObject = new LinkStateRefStruct();
  ...
  static createFromRefObject(refObject) {
    let self = new LinkStateWrapper();
    self.copyRefObject(refObject);
    return self;
  }
  static createArray() {
    return new LinkStateArrayWrapper;
  }
  static get ArrayType() {
    return LinkStateArrayWrapper;
  }
  static get refObjectArrayType() {
    return LinkStateRefStructArray
  }
  static get refObjectType() {
    return LinkStateRefStruct;
  }
  toRawROS() {
    this.freeze(true);
  ...
  freeze(own = false, checkConsistency = false) {
    if (checkConsistency) {
      if (!this._link_nameIntialized) {
        throw new TypeError('Invalid argument: link_name in LinkState');
      }
      if (!this._reference_frameIntialized) {
        throw new TypeError('Invalid argument: reference_frame in LinkState');
      }
    }
    if (own) {
  ...
  static destoryRawROS(msg) {
    LinkStateWrapper.freeStruct(msg.refObject);
  }
  static type() {
    return {pkgName: 'gazebo_msgs', subFolder: 'msg', interfaceName: 'LinkState'};
  }
  static isPrimitive() {
    return false;
  }
  ...
  copyRefObject(refObject) {
    this._refObject = new LinkStateRefStruct(refObject.toObject());
    this._link_nameIntialized = true;
...
class LinkStateArrayWrapper {
  ...
  fill(values) {
    ...
    values.forEach((value, index) => {
      if (value instanceof LinkStateWrapper) {
        ...
        this._wrappers[index] = new LinkStateWrapper(value);
      }
    });
  }
  ...
  set size(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to LinkStateArrayWrapper.size setter');
      return;
    }
  ...
  set capacity(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to LinkStateArrayWrapper.capacity setter');
    }
    return this._resize(value);
  }
  get refObject() {
  ...
  _resize(size) {
    ...
    this._refArray = new LinkStateRefArray(size);
    this._refObject = new LinkStateRefStructArray();
    this._refObject.size = size;
    this._refObject.capacity = size;
    this._wrappers = new Array();
    for (let i = 0; i < size; i++) {
      this._wrappers.push(new LinkStateWrapper());
    }
  }
  ...
  copy(other) {
    if (! (other instanceof LinkStateArrayWrapper)) {
      throw new TypeError('Invalid argument: should provide "LinkStateArrayWrapper".');
    }
  ...
  static freeArray(refObject) {
    ...
    for (let index = 0; index < refObject.size; index++) {
      LinkStateWrapper.freeStruct(refObjectArray[index]);
    }
  }
...
module.exports = LinkStateWrapper;
/*
